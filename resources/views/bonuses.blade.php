		       ...
		       <div class="col-12 col-lg-12">
				...
				<div class="card-header">
					<div class="d-flex align-items-center">
						<div>
							<h4 class="mb-0">Add New Bonus</h4>
						</div>
					</div>
				</div>
				  <div class="card-body">
				  	<div class="form-body">
					 <form class="row g-2" method="POST" action="submitbonus" id="submitbonus">
					 	@csrf
					 	<div class="row">
					 	<div class="col-sm-4">
		...
		<div class="card">
					<div class="card-body">
						<div class="card-title">
							<h4 class="mb-0">Available Bonuses</h4>
						</div>
						<hr/>
						<div class="table-responsive">
							<table id="example" class="table table-striped example">
								...
								<tbody>
									...
									<tr>
										<td>{{ $i++ }}</td>
										<td>{{ app\Http\Controllers\Controller::staffname($bonus->staff) }}</td>
										<td>{{ number_format($bonus->amount, 2) }}</td>
										<td>{{ $bonus->month }}</td>
										<td>{{ $bonus->bonus }}</td>
										@if($bonus->status == "Unpaid")
										<td><button class="btn btn-danger btn-sm" type="button">{{ $bonus->status }}</button></td>
										@else
										<td><button class="btn btn-success btn-sm" type="button">{{ $bonus->status }}</button></td>
										@endif
										<td>{{ $bonus->created_at }}</td>
										<td>
											
											<a href="{{ $bonus->id }}" class="editbonus" id="{{ $bonus->staff }}|{{ $bonus->bonus }}"  title="{{ $bonus->amount }}|{{ $bonus->month }}"><svg title="Click to edit row" class="vouchericon" id="editrow" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path fill="currentColor" d="m19.3 8.925l-4.25-4.2l1.4-1.4q.575-.575 1.413-.575t1.412.575l1.4 1.4q.575.575.6 1.388t-.55 1.387L19.3 8.925ZM17.85 10.4L7.25 21H3v-4.25l10.6-10.6l4.25 4.25Z"/></svg></a>

											<a href="{{ $bonus->id }}" class="deletebonus" id="{{ $bonus->id }}"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path fill="currentColor" d="M7 21q-.825 0-1.413-.588T5 19V6H4V4h5V3h6v1h5v2h-1v13q0 .825-.588 1.413T17 21H7ZM17 6H7v13h10V6ZM9 17h2V8H9v9Zm4 0h2V8h-2v9ZM7 6v13V6Z"/></svg></a>

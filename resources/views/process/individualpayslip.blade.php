<div class="card">
	<div class="card-body">
		<div class="card-title">
			<h4 class="mb-0">Staff Payslip for {{ app\Http\Controllers\Controller::staffname($payrolls[0]->staff) }}</h4>
		</div>
		<hr/>
		<div class="table-responsive">
			...
			<table class="table">
				...
				<tbody>
					...
					<tr>
						<td>{{ $i++ }}</td>
						<td><a href="{{ url('staffpayslip?staff='.$payroll->staff.'&month='.$payroll->month) }}" type="button" class="btn btn-primary btn-sm">{{ $payroll->month }}</a></td>
						<td>{{ app\Http\Controllers\Controller::staffname($payroll->staff) }}</td>
						<td>{{ app\Http\Controllers\Controller::getlevelname($payroll->designation) }}</td>
